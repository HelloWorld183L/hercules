...
def parse_fitnotes_csv(file_path: Path | str) -> list[WorkoutLogEntry]:
    ...
    for grouped_row_list in grouped_rows.values():
        ...
        try:
            ...
            logger.error(
                f"Skipping row. Error parsing row {row} in file {file_path}: {e}"
            )
            continue
        parsed_data.append(workout_log_entry)
...
def parse_fitnotes_db(db_path: Path | str) -> list[WorkoutLogEntry]:
    ...
    logger.warning(
        "FitNotes measurement units are not properly referred to in the database. Assuming all weights are in kilograms. Please verify this assumption."
    )
    ...
    for grouped_row_list in grouped_rows.values():
        ...
        try:
            ...
            logger.error(
                f"Skipping row. Error parsing row {row} in file {db_path}: {e}"
            )
            continue
        parsed_data.append(workout_log_entry)
    ...
    if len(parsed_data) == 0:
        logger.warning(
            f"No valid workout log entries could be parsed from database {db_path}."
        )
