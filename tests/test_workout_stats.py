...
def test_compute_workoutlog_stats_uses_days_in_gym_for_workout_consistency(
    sample_workout_entries: list[WorkoutLogEntry],
) -> None:
    """
...
def test_compute_workoutlog_stats_without_days_in_gym(
    sample_workout_entries: list[WorkoutLogEntry],
) -> None:
    """
...
def test_compute_workoutlog_stats_with_invalid_days_in_gym_raises_value_error(
    sample_workout_entries: list[WorkoutLogEntry],
) -> None:
    """
...
def test_compute_workoutlog_stats_with_bodyweight_adjustment(
    sample_bodyweight_entries: list[WorkoutLogEntry], bodyweight: float
) -> None:
    """
    ...
    stats = compute_workoutlog_stats(
        sample_bodyweight_entries, days_in_gym=3, bodyweight=bodyweight
    )
    ...
    assert stats.exercise_summary_stats is not None and all(
        entry.estimated_one_rep_max_progression_rate != None
        and entry.estimated_one_rep_max_progression_rate != None
        for entry in stats.exercise_summary_stats
    )
